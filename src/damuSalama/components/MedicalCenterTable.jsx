...
const MedicalCenterTable = () => {
  ...
  const getMedicalCenters = () => {
    return medicalCenters.slice(0, end)
  }

  useEffect(() => {
    setAllMedicalCenters(getMedicalCenters())
          ...
          <tbody>
            {allMedicalCenters.map((medicalCenter, index) => (
              ...
              >
                ...
                <td className={`py-2 px-4 text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{truncate(medicalCenter.MCPublicAddress, 7, 7, 17)}</td>
                <td className={`py-2 px-4 text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{medicalCenter.name}</td>
                <td className={`py-2 px-4 text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{medicalCenter.phoneNumber}</td>
